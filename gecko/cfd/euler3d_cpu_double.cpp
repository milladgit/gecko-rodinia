...
{
	double *p;
#pragma gecko memory allocate(p[0:N]) type(double) location(exec_loc)
	return p;
}
...
{
	int *p;
#pragma gecko memory allocate(p[0:N]) type(int) location(exec_loc)
	return p;
}
...
void dealloc_double(double *array) {
#pragma gecko memory free(array)
}

void dealloc_int(int *array) {
#pragma gecko memory free(array)
}
...
{
#pragma gecko region exec_pol(exec_policy_chosen) kernels
	for(int i = 0; i < N; i++)
	{
		dst[i] = src[i];
	}
...
void copy_double(double *dst, double *src, int N)
{
#pragma gecko region exec_pol(exec_policy_chosen) variable_list(dst,src) kernels
	for(int i = 0; i < N; i++)
	{
		dst[i] = src[i];
	}
...
{
#pragma gecko region exec_pol(exec_policy_chosen) variable_list(variables) independent collapse(2) gang vector
...
{
#pragma gecko region exec_pol(exec_policy_chosen) variable_list(variables,areas,step_factors) independent gang vector
	for(int i = 0; i < nelr; i++)
	{
...
#pragma gecko region exec_pol(exec_policy_chosen) variable_list(elements_surrounding_elements,normals,variables,fluxes) independent gang vector
	for(int i = 0; i < nelr; i++)
	{
		int j, nb;
...
{
#pragma gecko region exec_pol(exec_policy_chosen) variable_list(old_variables,variables,step_factors,fluxes) independent gang vector
    for(int i = 0; i < nelr; i++)
	{
...
//		normals = alloc<double>(NDIM*NNB*nelr);
	...
	double *variables = alloc_double(nelr*NVAR);
//	#pragma acc data create(variables[0:nelr*NVAR]) copyin(ff_variable)
	{
	initialize_variables(nelr, variables);
	} /* end pragma acc data */

	double *old_variables = alloc_double(nelr*NVAR);
	double *fluxes = alloc_double(nelr*NVAR);
	double *step_factors = alloc_double(nelr);
