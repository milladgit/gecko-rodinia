...

        {
            ...
            if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                #pragma omp parallel num_threads(jobCount)
                {
                    ...
                    geckoBindLocationToThread(devIndex, dev[devIndex]);
                    geckoSetDevice(dev[devIndex]);
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop deviceptr(iN,iS) async(asyncID)
...

        {
            ...
            if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                #pragma omp parallel num_threads(jobCount)
                {
                    ...
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop deviceptr(jW,jE) async(asyncID)
...

        {
            ...
            if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                #pragma omp parallel num_threads(jobCount)
                {
                    ...
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc serial deviceptr(iN,iS,jW,jE) async(asyncID)
                    for(int i = beginLI; i < endLI; i++)
                    {
                        iN[0]    = 0;
                        iS[rows-1] = rows-1;
...

        {
            ...
            if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                #pragma omp parallel num_threads(jobCount)
                {
                    ...
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop deviceptr(I,J) async(asyncID)
            ...
            sum2=0;
            {
                ...
                if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                    #pragma omp parallel num_threads(jobCount)
                    {
                        ...
                        int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                        int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop collapse(2) reduction(+:sum,sum2) deviceptr(I,J) async(asyncID)
...

            {
                ...
                if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                    #pragma omp parallel num_threads(jobCount)
                    {
                        ...
                        int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                        int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop collapse(2) independent private(k) deviceptr(I,J,c,dN,dS,dW,dE,iN,iS,jW,jE) async(asyncID)
                        for(int i = beginLI; i < endLI; i++) {
...

            {
                ...
                if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                    #pragma omp parallel num_threads(jobCount)
                    {
                        ...
                        int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                        int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop collapse(2) independent private(k) deviceptr(I,J,c,dN,dS,dW,dE,iS,jE) async(asyncID)
                        for(int i = beginLI; i < endLI; i++) {
                            for (int j = 0; j < cols; j++) {
