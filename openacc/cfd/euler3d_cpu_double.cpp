...
{
	gecko_double p;
#pragma gecko memory allocate(p[0:N]) type(gecko_double) location(exec_loc)
	return p;
}
...
{
	gecko_int p;
#pragma gecko memory allocate(p[0:N]) type(gecko_int) location(exec_loc)
	return p;
}
...
#pragma gecko memory freeobj(array)
}
...
#pragma gecko memory allocate(ff_variable[0:NVAR]) type(gecko_double) location(exec_loc)
