...
//	return new T[N];
    T* p;
    geckoMemoryDeclare((void**)&p, sizeof(T), N, exec_loc, GECKO_DISTANCE_NOT_SET);
    return p;
}

gecko_double alloc_double(int N)
{
    gecko_double p;
    geckoMemoryInternalTypeDeclare(p, sizeof(double), N, exec_loc, GECKO_DISTANCE_NOT_SET);
    return p;
}

gecko_int alloc_int(int N)
{
    gecko_int p;
    geckoMemoryInternalTypeDeclare(p, sizeof(int), N, exec_loc, GECKO_DISTANCE_NOT_SET);
    return p;
}
...
void dealloc_double(gecko_double array) {
    array.freeMem();
}

void dealloc_int(gecko_int array) {
    array.freeMem();
}
...
{
    {
        ...
        int var_count = 0;
        void **var_list = NULL;
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, N, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
...
#pragma acc kernels deviceptr() async(asyncID)
...
void dump(gecko_double variables, int nel, int nelr)
{
...
{
    {
        ...
        int var_count = 0;
        void **var_list = NULL;
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, nelr, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop collapse(2) independent deviceptr() async(asyncID)
                    for(int i = beginLI; i < endLI; i++)
                    {
...
void compute_flux(int nelr, gecko_int elements_surrounding_elements, gecko_double normals, gecko_double variables, gecko_double fluxes)
{
...
int main(int argc, char** argv)
{
    ...
    geckoMemoryInternalTypeDeclare(ff_variable, sizeof(double), NVAR, exec_loc, GECKO_DISTANCE_NOT_SET);
    ...
    gecko_double areas;
    gecko_int elements_surrounding_elements;
    gecko_double normals;
    {
...
//		normals = alloc<double>(NDIM*NNB*nelr);
        areas = alloc_double(nelr);
        elements_surrounding_elements = alloc_int(nelr*NNB);
        normals = alloc_double(NDIM*NNB*nelr);
    ...
    gecko_double variables = alloc_double(nelr*NVAR);
//	#pragma acc data create(variables[0:nelr*NVAR]) copyin(ff_variable)
    {
        initialize_variables(nelr, variables);
    } /* end pragma acc data */

    gecko_double old_variables = alloc_double(nelr*NVAR);
    gecko_double fluxes = alloc_double(nelr*NVAR);
    gecko_double step_factors = alloc_double(nelr);
...
//		copyin(normals[0:NDIM*NNB*nelr]) copyout(variables[0:nelr*NVAR])
    ...
    dealloc_double(areas);
    dealloc_int(elements_surrounding_elements);
    dealloc_double(normals);

    dealloc_double(variables);
    dealloc_double(old_variables);
    dealloc_double(fluxes);
    dealloc_double(step_factors);
