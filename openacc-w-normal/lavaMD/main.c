...
char *exec_loc = "LocH";
char *exec_policy_chosen = "runtime";
...
//	box_cpu = (box_str*)malloc(dim_cpu.box_mem);
	int dim_count = dim_cpu.number_boxes;
	#pragma gecko memory allocate(box_cpu[0:dim_count]) type(box_str) location(exec_loc)
...
//	rv_cpu = (FOUR_VECTOR*)malloc(dim_cpu.space_mem);
	dim_count = dim_cpu.space_elem;
#pragma gecko memory allocate(rv_cpu[0:dim_count]) type(FOUR_VECTOR) location(exec_loc)
...
#pragma gecko memory allocate(qv_cpu[0:dim_count]) type(fp) location(exec_loc)
	for(i=0; i<dim_cpu.space_elem; i=i+1){
...
#pragma gecko memory allocate(fv_cpu[0:dim_count]) type(FOUR_VECTOR) location(exec_loc)
	for(i=0; i<dim_cpu.space_elem; i=i+1){
...
#pragma gecko memory free(rv_cpu,qv_cpu,fv_cpu,box_cpu)
