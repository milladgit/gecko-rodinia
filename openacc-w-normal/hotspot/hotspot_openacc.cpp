...
//		fatal("unable to allocate memory");
	...
	pfile = argv[5];
	double *ttt;
	ttt = (double *) calloc (grid_rows * grid_cols, sizeof(double));
	#pragma gecko memory register(ttt[0:grid_rows*grid_cols]) type(double) loc("LocN")

	read_input(ttt, grid_rows, grid_cols, tfile);
	#pragma gecko memory copy from(ttt[0:grid_rows*grid_cols]) to(temp[0:grid_rows*grid_cols])
	read_input(ttt, grid_rows, grid_cols, pfile);
	#pragma gecko memory copy from(ttt[0:grid_rows*grid_cols]) to(power[0:grid_rows*grid_cols])
	...
	#pragma gecko memory copy to(ttt[0:grid_rows*grid_cols]) from(temp[0:grid_rows*grid_cols])
	/* output results	*/
#ifdef VERBOSE
...
#pragma gecko memory unregister(ttt)
#pragma gecko memory free(temp,power,result)
