...
#pragma acc kernels present_or_copyin(src[0:N]) present_or_create(dst[0:N]) deviceptr(src,dst) async(asyncID) copyin()
...
{
    {
        ...
        void **var_list = (void **) malloc(sizeof(void*) * var_count);
        for(int __v_id=0; __v_id<var_count; __v_id++) {
            var_list[__v_id] = variables;
            var_list[__v_id] = ff_variable;
        }
        ...
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop collapse(2) independent deviceptr(variables,ff_variable) async(asyncID) copyin()
                    for(int i = beginLI; i < endLI; i++)
                    {
        ...
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop independent deviceptr(variables,ff_variable,step_factors) async(asyncID) copyin()
                    for(int i = beginLI; i < endLI; i++)
                    {
...
void compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
...

    {
        ...
        void **var_list = (void **) malloc(sizeof(void*) * var_count);
        for(int __v_id=0; __v_id<var_count; __v_id++) {
            var_list[__v_id] = elements_surrounding_elements;
            var_list[__v_id] = variables;
            var_list[__v_id] = ff_variable;
            var_list[__v_id] = normals;
            var_list[__v_id] = fluxes;
        }
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, nelr, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop independent deviceptr(elements_surrounding_elements,variables,ff_variable,normals,fluxes) async(asyncID) copyin()
                    for(int i = beginLI; i < endLI; i++)
                    {
                        int j, nb;
                        double3 normal;
        ...
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop independent deviceptr(old_variables,variables,step_factors,fluxes) async(asyncID) copyin()
                    for(int i = beginLI; i < endLI; i++)
                    {
...
int main(int argc, char** argv)
{
    ...
    geckoMemoryDeclare((void**)&ff_variable, sizeof(double), NVAR, exec_loc, GECKO_DISTANCE_NOT_SET);
