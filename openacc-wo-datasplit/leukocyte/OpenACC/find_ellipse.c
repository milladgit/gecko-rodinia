...
void compute_constants() {
    ...
    //  (which are the same across all sample circles)
    {
        ...
        for(int __v_id=0; __v_id<var_count; __v_id++) {
            var_list[__v_id] = theta;
            var_list[__v_id] = sin_angle;
            var_list[__v_id] = cos_angle;
        }
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, NPOINTS, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
...
#pragma acc kernels deviceptr(theta,sin_angle,cos_angle) async(asyncID) copyin()
                    for( n = beginLI; n < endLI; n++) {
...
#pragma acc wait(asyncID)
    ...
    // Compute the (x,y) pixel offsets of each sample point in each sample circle
    {
        ...
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, NCIRCLES, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc kernels deviceptr(tX,tY,theta) async(asyncID) copyin()
...
#pragma acc wait(asyncID)
    ...
    // Compute the structuring element used in dilation
    {
        ...
        void **var_list = (void **) malloc(sizeof(void*) * var_count);
        for(int __v_id=0; __v_id<var_count; __v_id++) {
            var_list[__v_id] = strel;
        }
        GeckoError err = geckoRegion(exec_policy_chosen, exec_loc, 0, strel_m, 1, 0, &devCount, &beginLoopIndex, &endLoopIndex, &dev, ranges_count, ranges, var_count, var_list);
        jobCount = devCount;
        if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
            #pragma omp parallel num_threads(jobCount)
            {
                ...
                if(dev[devIndex] != NULL) {
                    int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                    int asyncID = dev[devIndex]->getAsyncID();
#pragma acc kernels deviceptr(strel) async(asyncID) copyin()
