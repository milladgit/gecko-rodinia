...
//	mem = (char*)malloc(malloc_size);
    geckoMemoryDeclare((void**)&mem, sizeof(char), malloc_size, exec_loc, GECKO_DISTANCE_NOT_SET);
...
//				long *currKnode = (long *)malloc(count*sizeof(long));
            long *currKnode;
            geckoMemoryDeclare((void**)&currKnode, sizeof(long), count, exec_loc, GECKO_DISTANCE_NOT_SET);

            // INPUT: offset CPU allocation, initialize in GPU
//				long *offset = (long *)malloc(count*sizeof(long));
            long *offset;
            geckoMemoryDeclare((void**)&offset, sizeof(long), count, exec_loc, GECKO_DISTANCE_NOT_SET);

            // INPUT: keys CPU allocation
//				int *keys = (int *)malloc(count*sizeof(int));
            int *keys;
            geckoMemoryDeclare((void**)&keys, sizeof(int), count, exec_loc, GECKO_DISTANCE_NOT_SET);
...
//				record *ans = (record *)malloc(sizeof(record)*count);
            record *ans;
            geckoMemoryDeclare((void**)&ans, sizeof(record), count, exec_loc, GECKO_DISTANCE_NOT_SET);
...
#pragma acc parallel loop deviceptr(currKnode,offset,ans) async(asyncID) copyin()
...
//				long *lastKnode = (long *)malloc(count*sizeof(long));
            long *lastKnode;
            geckoMemoryDeclare((void**)&lastKnode, sizeof(long), count, exec_loc, GECKO_DISTANCE_NOT_SET);

            // INPUT: offset_2 CPU allocation, initialize in GPU
//				long *offset_2 = (long *)malloc(count*sizeof(long));
            long *offset_2;
            geckoMemoryDeclare((void**)&offset_2, sizeof(long), count, exec_loc, GECKO_DISTANCE_NOT_SET);

            // INPUT: start, end CPU allocation
//				int *start = (int *)malloc(count*sizeof(int));
            int *start;
            geckoMemoryDeclare((void**)&start, sizeof(int), count, exec_loc, GECKO_DISTANCE_NOT_SET);

//				int *end = (int *)malloc(count*sizeof(int));
            int *end;
            geckoMemoryDeclare((void**)&end, sizeof(int), count, exec_loc, GECKO_DISTANCE_NOT_SET);
...
//				int *recstart = (int *)malloc(count*sizeof(int));
            int *recstart;
            geckoMemoryDeclare((void**)&recstart, sizeof(int), count, exec_loc, GECKO_DISTANCE_NOT_SET);
//				int *reclength = (int *)malloc(count*sizeof(int));
            int *reclength;
            geckoMemoryDeclare((void**)&reclength, sizeof(int), count, exec_loc, GECKO_DISTANCE_NOT_SET);
                    ...
                    if(err != GECKO_ERR_TOTAL_ITERATIONS_ZERO) {
                        #pragma omp parallel num_threads(jobCount)
                        {
                            ...
                            if(dev[devIndex] != NULL) {
                                int beginLI = beginLoopIndex[devIndex], endLI = endLoopIndex[devIndex];
                                int asyncID = dev[devIndex]->getAsyncID();
#pragma acc parallel loop deviceptr(currKnode,offset,lastKnode,offset_2,recstart,reclength) async(asyncID) copyin()
                                for( i = beginLI; i < endLI; i++) {
                                    currKnode[i] = 0;
                                    offset[i] = 0;
                                    lastKnode[i] = 0;
